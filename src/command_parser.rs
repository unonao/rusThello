...
pub enum Message {
    Open{name:String},
    Start{color:String, name:String, time:i32},
    End {win_lose:String, n:i32,  m:i32, reason:String},
    Bye{stats:Vec<Stat>},
    Move {x:i32, y:i32},
    Pass,
    Giveup,
...
fn move_parse(input: &str) -> IResult<&str,Message> { // Move{x:i32, y:i32}
...
fn ack_parse(input: &str) -> IResult<&str,Message> { //Ack{time:i32}
